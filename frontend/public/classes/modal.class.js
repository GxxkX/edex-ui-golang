// Wails 版本的模态框类
// 从 Electron modal.class.js 转换而来
...
class Modal {
    ...
    // 生成唯一 ID（替代 nanoid）
    generateId() {
        return 'modal_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
    }
    ...
    init() {
        ...
        let DOMstring = `<div id="modal_${this.id}" class="${this.classes}" style="z-index:${zindex+Object.keys(window.modals).length};" augmented-ui="${augs.join(" ")} exe">
            <h1>${this.title}</h1>
            ${this.type === "custom" ? this.options.html : "<h5>"+this.message+"</h5>"}
            <div>`;
            buttons.forEach(b => {
